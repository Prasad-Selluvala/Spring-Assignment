...
public class EcartDaoImpl implements EcartDao {
	...
	public List<Orders> getOrderByUserId(int userId) {
		...
		Query query = sf.getCurrentSession().createQuery("from Orders where user_id = :userId");
	...
	public Product getProductById(int pId) {
		...
		Query query = sf.getCurrentSession().createQuery("from Product where product_id = :productId");
		query.setInteger("productId", pId);
	...
	public List<Product> getProductByName(String name) {
		...
		Query query = sf.getCurrentSession().createQuery("from Product where productName = :productName");
	...
	public List<Review> listReviewByProductId(int pId) {
		Query query = sf.getCurrentSession().createQuery("from Review where product_id = :productId");
		query.setInteger("productId", pId);
		Object queryResult = query.getResultList();
	...
	public Review updateReview(int pId, int userId) {
		Query query = sf.getCurrentSession().createQuery("from Review where product_id = :productId and user_id = :userId");
		query.setInteger("productId", pId);
		query.setInteger("userId", userId);
