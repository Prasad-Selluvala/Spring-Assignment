...
public class ProductDao {
	...
	public List<Product> findByPriceRange(int i, int j) {
		List<Product> prod1 = jt.query("select * from product where price between ? and ? ",
				new ResultSetExtractorDemoByPriceRange(), i, j);
		return prod1;
	}

	public List<Product> findBetweenId(int i, int j) {
		List<Product> prod2 = (List<Product>) jt.query("select * from product where product_id between ? and ?",
